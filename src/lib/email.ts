...
async function getAccessToken(): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error("Failed to get Microsoft access token: " + err);
  }
...
export async function sendEmail({ to, subject, html }: EmailOptions): Promise<void> {
  ...
  const res = await fetch(
    "https://graph.microsoft.com/v1.0/users/" + env.MS_SENDER_EMAIL + "/sendMail",
    {
      method: "POST",
      headers: {
...
export function passwordResetEmail(firstName: string, resetUrl: string): string {
  return '<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;padding:20px;background:#f7f7f7">' +
    ...
    '</div>' +
    '<p style="color:#888;font-size:12px;line-height:1.5">If you didn\'t request this, you can safely ignore this email. Your password won\'t change.</p>' +
